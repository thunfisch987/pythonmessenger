    ...
    while True:
        ...
        if self.incmessage.name != "":
            if addr[0] == "127.0.0.1":
                self.ausgabe.insert(
                    tk.END,
                    f"[{current_time}] {self.incmessage.name} (You): '{self.incmessage.msg}' \n",
                )
            else:
                self.ausgabe.insert(
                    tk.END,
                    f"[{current_time}] {self.incmessage.name} : '{self.incmessage.msg}' \n",
                )
...
def send(self) -> None:
    ...
    self.message = self.Message(name=self.name_Entry.get(), msg=self.msg_Entry.get())
    # senden der Nachricht an Ziel-Adresse über Socket
    self.serversocket.sendto(self.message.encoded(), (self.empfaenger, self.port))
    # löschen des Inhalts des Eingabe-Widgets
    self.ausgabe.configure(state="normal")
    if self.message.name == "":
        self.ausgabe.insert(tk.END, f"You: '{self.message.msg}'\n", "right")
    else:
        self.ausgabe.insert(
