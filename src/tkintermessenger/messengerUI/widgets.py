...
class Widgets:
    ...
    def widgetmaking(self, port: int):
        ...
        self.parent.ausgabe.grid(row=3, column=0, columnspan=4, sticky="ew")
        ...
        self.parent.send_Button.grid(row=4, column=3, sticky="w")
    ...
    def makeMessageWidgets(self):
        ...
        self.parent.message_label.grid(row=4, column=0, sticky="w")
        self.parent.msg_Entry.grid(row=4, column=1, sticky="w")
        self.parent.length_Label.grid(row=4, column=2, sticky="w")

        self.parent.msg_Entry.bind("<Return>", self.parent.sendmessage)
        return
    ...
    def usernamewidget(self):
        ...
        self.parent.name_Entry["validatecommand"] = (self.parent.usernamecmd, "%P")
        ...
        self.parent.topmost_Button = ttk.Button(
            self.parent.UsernameFrame,
            text="AlwaysOnTop",
            command=self.parent.settings.makeTopMost,
            style="topmost.TButton",
        )

        self.parent.UsernameFrame.grid(row=0, column=0, columnspan=4, sticky="nsew")
    ...
    def makeIPFrame(self, port: int):
        ...
        self.parent.ip_Entry_1["width"] = self.parent.ip_Entry_2[
            "width"
        ] = self.parent.ip_Entry_3["width"] = self.parent.ip_Entry_4["width"] = 5
        self.parent.ip_Entry_1["validate"] = self.parent.ip_Entry_2[
            "validate"
        ] = self.parent.ip_Entry_3["validate"] = self.parent.ip_Entry_4[
            "validate"
        ] = "key"
        self.parent.ip_Entry_1["validatecommand"] = self.parent.ip_Entry_2[
            "validatecommand"
        ] = self.parent.ip_Entry_3["validatecommand"] = self.parent.ip_Entry_4[
            "validatecommand"
        ] = (
            self.parent.vcmd,
            "%P",
        ...
        self.parent.BigFrame.grid(row=1, column=0, columnspan=4, sticky="nsew")
    ...
    def validmessage(self, message) -> bool:
        self.parent.lengthvar.set("Remaining Characters: " + str(1000 - len(message)))
...
class Settings:
    ...
    def makeTopMost(self) -> None:
        ...
        if self.parent.wm_attributes("-topmost") == 1:
            ...
            self.parent.buttonstyle.configure("topmost.TButton", background="palegreen")
        return
